...
# Crear Convolutional Neuron Network CNN with Tensorflow 
...
cnn.fit(image_entren, steps_per_epoch=pasos, epochs=epocas,
        validation_data=image_valid, validation_steps=pasos_validacion)


def save_model(ruta,model):
  if not os.path.exists(ruta):
     os.mkdir(ruta)
     model.save(ruta+'/model.h5')

save_model('./models',model=cnn)
